...
| 2 | No server-side enforcement of plan limits | ⬜ Open | `plan` can now genuinely be "pro" (see the admin panel fix in the Security/Auth section — that solved *changing* it), but nothing checks `plan` anywhere to actually behave differently — every account has identical capabilities regardless of value. Changing the plan and enforcing something because of the plan are two separate gaps; only the first is fixed. |
